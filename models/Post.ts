...
import {Writeable} from '../utils/typescript-utils';
...
import {BaseAccessArgs, isAuthenticated} from '../utils/access-control-utils';
import {FilterOutput} from '../utils/keystone-utils';
...
export const Post = list({
    ...
    publishedAt: timestamp({
      ...
      hooks: {
        resolveInput: async (args) => {
          ...
          const newResolvedData: Writeable<PostCreateInput | PostUpdateInput> = {
            ...resolvedData,
          };
  ...
  access: {
    filter: {
      query: (args: BaseAccessArgs<Lists.Post.TypeInfo>): FilterOutput<Lists.Post.TypeInfo> => isAuthenticated(args) ? true : {status: {equals: 'published'}},
    },
    operation: {
